...
class ExamPrompts:
    ...
    def get_system_prompts(exam_name: str = "정보시스템감리사"):
        ...
        return {
            "question_generator": f"당신은 {exam_name} 시험 전문가입니다. 실제 기출문제와 유사한 수준의 문제를 생성해주세요. 저작권이나 정책 관련 언급을 절대 하지 마세요.",
...
당신의 역할은 기출문제의 품질을 보장하고 사용자에게 정확한 정보를 제공하는 것입니다."""
    ...
    def get_question_generation_prompt(subject: str, difficulty: str, question_type: str, exam_name: str = "정보시스템감리사") -> str:
        ...
        **중요 사항**:
        ...
        4. **저작권 관련 메시지 금지**: 저작권이나 정책 관련 언급을 절대 하지 마세요.
        5. **문제 영역 분리**: === 문제 === 영역에는 문제 내용만 포함하고, 정답이나 해설은 절대 포함하지 마세요.
        6. **추가 설명 금지**: 문제 영역에 추가적인 설명이나 안내 문구를 포함하지 마세요.
        
        다음 형식으로 응답해주세요:
        === 문제 ===
        [문제 내용만 - 정답이나 해설은 절대 포함하지 않음]
    ...
    def get_rag_question_generation_prompt(subject: str, difficulty: str, question_type: str, 
        ...
        **중요 사항**:
        ...
        6. **저작권 관련 메시지 금지**: 저작권이나 정책 관련 언급을 절대 하지 마세요.
        7. **문제 영역 분리**: === 문제 === 영역에는 문제 내용만 포함하고, 정답이나 해설은 절대 포함하지 마세요.
        8. **추가 설명 금지**: 문제 영역에 추가적인 설명이나 안내 문구를 포함하지 마세요.
        9. 메타데이터 정보를 참고하여 정확한 출처 정보를 문제에 포함시켜주세요.
    ...
    def get_exact_question_prompt(context: str, exam_name: str = "정보시스템감리사") -> str:
        ...
        **중요 사항**:
        ...
        4. **저작권 관련 메시지 금지**: 저작권이나 정책 관련 언급을 절대 하지 마세요.
        5. **문제 영역 분리**: === 문제 === 영역에는 문제 내용만 포함하고, 정답이나 해설은 절대 포함하지 마세요.
        6. **추가 설명 금지**: 문제 영역에 "이와 같은 형식으로 추가 문제 생성이 가능합니다" 같은 추가 설명을 포함하지 마세요.
        7. **안전한 내용만 생성**: 폭력, 위험, 해로운 내용은 포함하지 말고 교육적이고 건설적인 내용만 다루세요.
        8. **학습 목적**: 이는 교육 및 학습 목적으로만 사용되는 시험 문제입니다.
        
        다음 형식으로 응답해주세요:
        === 문제 ===
        [기출문제 내용만 - 표가 있다면 표도 포함, 정답이나 해설은 절대 포함하지 않음]
